  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<CartBloc, CartState>(
      ...
      listener: (context, state) {
        if (state.checkoutStatus.isSuccess) {
          if (state.removedCount > 0) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  context.tr(
                    LocaleKeys.items_removed_from_cart,
                    args: [state.removedCount.toString()],
                  ),
                ),
                backgroundColor: Colors.orange,
              ),
            );
            // Don't return — fall through to navigate if carts still has items
          }
        ...
        body: BlocBuilder<CartBloc, CartState>(
          builder: (context, state) {
            ...
            if (state.carts.isEmpty) {
              return Center(
                child: Text(context.tr(LocaleKeys.empty_card_text)),
              );
            }
              ...
              child: Padding(
                ...
                child: CustomElevatedButton(
                  onTap: () {
                    ...
                    bloc.add(CartValidateAndCheckout());
                  },
